...
export default function Board() {
  ...
  const [players, setPlayers] = useState<PlayerType[]>([
    { id: 1, position: 1, emoji: "👨", color: "text-blue-500" },
    { id: 2, position: 1, emoji: "👩", color: "text-pink-500" },
  ])
  ...
  useEffect(() => {
    ...
    const getCoordinates = (number: number) => {
      ...
      const col =
        row % 2 === 0 ? (number % 10 === 0 ? 9 : (number % 10) - 1) : number % 10 === 0 ? 0 : 10 - (number % 10)
      return {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 bg-gray-100 p-4 dark:bg-gray-900">
      ...
      <div className="flex items-center gap-4">
        <div className="text-center">
          <h2 className={`mb-2 text-lg font-bold ${players[0].color}`}>
      ...
      <div className="relative w-full max-w-2xl">
        <div className="grid grid-cols-10 gap-0.5 rounded-lg border bg-white p-2 dark:border-gray-800 dark:bg-gray-800">
          {squares.map((number) => {
            ...
            return (
              ...
              >
                {number}
                {isSnakeStart && <span className="absolute -top-1 right-0 text-red-500">🐍</span>}
                {isLadderStart && <span className="absolute -top-1 right-0 text-green-500">🪜</span>}
                {playersHere.length > 0 && (
                  <div className="absolute bottom-0 left-0 flex">
                    {playersHere.map((player) => (
                      <span key={player.id} className={player.color}>
                        {player.emoji}
                      </span>
                    ))}
                  </div>
        ...
        <canvas ref={canvasRef} width={500} height={500} className="absolute left-0 top-0 h-full w-full" />
      </div>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        {gameWon && (
          <div className="text-center">
            <h2 className="text-2xl font-bold">Player {currentPlayer === 1 ? 2 : 1} Wins! 🎉</h2>
            <button
              onClick={() => {
                setPlayers([
                  { id: 1, position: 1, emoji: "👨", color: "text-blue-500" },
